...
# see the *Quick start: Hello world* in the documentation.
#
...
# %% [markdown]
# ## Analyze spatial mixing patterns
#
...
#
# **For programmatic use** — both model types accept a mixer object via the same `InfectionParams(mixer=...)` kwarg:
#
# - **Compartmental and ABM:** `InfectionParams(mixer=GravityMixing(...))` or `InfectionParams(mixer=RadiationMixing(...))`. Inspect the resulting matrix via `mixer.get_migration_matrix()`.
# - **ABM legacy / default-gravity path:** if you leave `mixer=None` on the ABM's `InfectionParams`, it falls back to building a `GravityMixing` internally from `distance_exponent` and `mixing_scale`. When `mixer=` is set explicitly, those legacy parameters are ignored.
# - **Custom mixers (radiation for the ABM, chain mixers, etc.)** are passed via the same `mixer=` kwarg by constructing a `BaseMixing` subclass or using a stock one. The model assigns the patch scenario to the mixer automatically; callers don't set `mixer.scenario` themselves.
